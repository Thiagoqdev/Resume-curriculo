...
from utils.generic_parses import _parse_salary
from utils.maps import contract_map, field_map
...
class SQLRepository:
    ...
    def __init__(self):
        ...
        if isinstance(conn_str, str) and conn_str.startswith("mssql+aioodbc://"):
            sync_conn_str = conn_str.replace("mssql+aioodbc://", "mssql+pyodbc://", 1)
        else:
            sync_conn_str = conn_str
...
class JobRepository(SQLRepository):
    """Repositório para descrições de vaga"""

    async def add(self, job: 'JobDescription', created_by_user_uuid: Optional[str] = None) -> 'JobDescription':
        """Insere uma nova descrição de vaga na tabela `jobs.tb_jobs`."""
        query = """
        INSERT INTO jobs.tb_jobs (
            job_uuid, company_id, job_title, contract_type_id, location_requirement_id,
            job_location, job_text_description, min_pay, max_pay, is_active,
        ...
        min_pay, max_pay = _parse_salary(getattr(job, 'salary_range', None))

        company_id_val = await resolve_id_from_uuid(
                        self,
                        job.company_id,
                        "companies.tb_companies",
                        "company_uuid",
                        "company_id"
                    )
        ...
        params = {
            ...
            'job_title': job.title,
            'contract_type_id': contract_map.get(getattr(job.job_type, 'value', None), None),
            'location_requirement_id': None,
            'job_location': job.location,
            'job_text_description': job.description,
            'min_pay': min_pay,
            'max_pay': max_pay,
        ...
        created_by_user_id_val =  await resolve_id_from_uuid(
                    self,
        ...
        params['created_by_user_id'] = created_by_user_id_val
        ...
        try:
            ...
            logger.error(f"Error creating job description: {e}", exc_info=True)
            raise

    async def get_by_id(self, job_id: UUID) -> Optional['JobDescription']:
        """Busca uma descrição de vaga pelo ID."""
        query = """
         SELECT job_uuid, company_id, job_title, contract_type_id, location_requirement_id,
             job_location, job_text_description, min_pay, max_pay, is_active, created_by_user_id, created_at
         FROM jobs.tb_jobs
         WHERE job_uuid = :job_uuid
         """

        result = self.execute_query(query, {"job_uuid": str(job_id)})
        if not result:
            return None

        row = result[0]
        return JobDescription(
            job_id=UUID(row["job_uuid"]),
            company_id=None,
            ...
            description=row.get("job_text_description"),
            requirements=None,
            benefits=None,
    ...
    async def update(self, job_id: UUID, update_data: dict) -> Optional['JobDescription']:
        """Atualiza uma descrição de vaga existente."""
        set_clauses = []
        params = {"job_uuid": str(job_id)}
        ...
        if 'job_type' in update_data and update_data.get('job_type') is not None:
            params['contract_type_id'] = contract_map.get(getattr(update_data['job_type'], 'value', update_data['job_type']), None)
            set_clauses.append('contract_type_id = :contract_type_id')


        if 'salary_range' in update_data and update_data.get('salary_range') is not None:
            min_pay, max_pay = _parse_salary(update_data.get('salary_range'))
            if min_pay is not None:
                set_clauses.append('min_pay = :min_pay')
                params['min_pay'] = min_pay
            if max_pay is not None:
                set_clauses.append('max_pay = :max_pay')
        ...
        try:
            ...
            logger.error(f"Error updating job description: {e}", exc_info=True)
            raise

    async def delete(self, job_id: UUID) -> bool:
        """Exclui uma descrição de vaga pelo ID."""
        query = """
        DELETE FROM jobs.tb_jobs
        WHERE job_uuid = :job_uuid
        """
        try:
            with self.get_session() as session:
                result = session.execute(text(query), {"job_uuid": str(job_id)})
                session.commit()
                return result.rowcount > 0
    ...
    async def list_by_creator_id(self, creator_id: int) -> List[JobDescription]:
        query = """
         SELECT job_uuid, company_id, job_title, contract_type_id, location_requirement_id,
             job_location, job_text_description, min_pay, max_pay, is_active, created_by_user_id, created_at
         FROM jobs.tb_jobs
